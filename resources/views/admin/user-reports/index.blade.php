...
<div class="container mx-auto px-4 py-6">
    <div class="mb-6 flex justify-between items-center">
        <div>
            <h1 class="text-3xl font-black tracking-tight text-slate-950 dark:text-white">User Reports</h1>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow mb-6 p-4 border border-transparent dark:border-gray-700">
        <form method="GET" action="{{ route('admin.user-reports.index') }}" class="flex flex-wrap gap-4">
            <div class="flex-1 min-w-[200px]">
                ...
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search by user name, email, or reason..." class="w-full bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-slate-950 dark:text-white">
            </div>
            <div class="w-48">
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Status</label>
                <select name="status" class="w-full bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-slate-950 dark:text-white">
            ...
            <div class="w-48">
                ...
                <select name="type" class="w-full bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-slate-950 dark:text-white">
    ...
    <div class="bg-white dark:bg-gray-800 shadow overflow-hidden sm:rounded-lg border border-transparent dark:border-gray-700">
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            <thead class="bg-gray-50 dark:bg-gray-900/50">
                <tr>
                    <th scope="col" class="px-6 py-3 text-left text-xs font-black uppercase tracking-wider text-slate-950 dark:text-white">ID</th>
                    <th scope="col" class="px-6 py-3 text-left text-xs font-black uppercase tracking-wider text-slate-950 dark:text-white">Reported User</th>
                    <th scope="col" class="px-6 py-3 text-left text-xs font-black uppercase tracking-wider text-slate-950 dark:text-white">Reported By</th>
                    <th scope="col" class="px-6 py-3 text-left text-xs font-black uppercase tracking-wider text-slate-950 dark:text-white">Type</th>
                    <th scope="col" class="px-6 py-3 text-left text-xs font-black uppercase tracking-wider text-slate-950 dark:text-white">Status</th>
                    <th scope="col" class="px-6 py-3 text-left text-xs font-black uppercase tracking-wider text-slate-950 dark:text-white">Date</th>
                    <th scope="col" class="px-6 py-3 text-left text-xs font-black uppercase tracking-wider text-slate-950 dark:text-white">Actions</th>
                </tr>
            </thead>
            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                @forelse($reports as $report)
                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-black text-slate-950 dark:text-white">#{{ $report->id }}</td>
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="flex items-center">
                            ...
                            @endif
                            <div class="ml-3">
                                <div class="text-sm font-black text-slate-950 dark:text-white">{{ $report->reported->fullName() ?? 'Unknown User' }}</div>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                        <div class="flex items-center">
                            ...
                            @endif
                            <div>
                                <div class="font-black text-slate-950 dark:text-white">{{ $report->reporter->fullName() ?? 'Unknown' }}</div>
