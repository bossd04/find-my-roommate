                ...
                <nav class="mt-2 flex-1 px-3 space-y-1">
                   ...
                   <!-- Listings -->
                    ...
                    @if(auth('admin')->user()->is_superadmin)
                        <!-- Divider -->
                        <div class="py-4 px-4">
                            <div class="h-[1px] bg-purple-500/30"></div>
                        </div>

                        <!-- Super Admin Section -->
                        <div class="px-4 py-2">
                            <span class="text-[10px] font-black uppercase tracking-[0.2em] text-purple-400">Super Admin</span>
                        </div>

                        <a href="{{ route('admin.superadmin.dashboard') }}"
                           class="group flex items-center px-4 py-3 text-xs font-black uppercase tracking-widest rounded-xl transition-all duration-300 {{ request()->routeIs('admin.superadmin.dashboard') ? 'bg-purple-500 text-white shadow-lg' : 'text-purple-200 hover:bg-purple-500/20 hover:text-white' }}">
                            <i class="fas fa-crown mr-4 text-purple-400 group-hover:text-white transition-colors"></i>
                            Super Dashboard
                        </a>

                        <a href="{{ route('admin.superadmin.admins') }}"
                           class="group flex items-center px-4 py-3 text-xs font-black uppercase tracking-widest rounded-xl transition-all duration-300 {{ request()->routeIs('admin.superadmin.admins') || request()->routeIs('admin.superadmin.create-admin') ? 'bg-purple-500 text-white shadow-lg' : 'text-purple-200 hover:bg-purple-500/20 hover:text-white' }}">
                            <i class="fas fa-user-shield mr-4 text-purple-400 group-hover:text-white transition-colors"></i>
                            Manage Admins
                        </a>
                    @endif
                </nav>
