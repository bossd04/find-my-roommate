                    ...
                    <div class="relative" x-data="{ open: false }">
                        <button @click="open = !open" class="flex items-center space-x-4 p-1 rounded-lg transition-all focus:outline-none">
                            <div class="flex flex-col text-right hidden sm:block">
                                <span class="text-sm font-bold text-gray-900 dark:text-white">{{ auth()->user()->is_superadmin ? 'Superadmin' : 'Admin' }}</span>
                            </div>
                            <div class="h-10 w-10 rounded-full bg-indigo-100 border-2 border-indigo-200 overflow-hidden shadow-sm">
                                @if(auth()->user()->profile_photo_path)
                                    <img src="{{ route('profile.photo.serve', ['filename' => basename(auth()->user()->profile_photo_path)]) }}" alt="Superadmin" class="h-full w-full object-cover nav-avatar">
                                @else
