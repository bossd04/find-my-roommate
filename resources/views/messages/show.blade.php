            ...
            <div class="flex" style="height: calc(100vh - 200px);">
                ...
                <div class="w-1/3 border-r border-gray-700 bg-gray-800 flex flex-col">
                    ...
                    <div class="flex-1 flex flex-col h-full overflow-hidden">
                        <!-- Chat header -->
                        <div class="p-4 border-b border-gray-700 bg-gray-800 flex items-center shadow-sm">
                            ...
                            <div class="flex-shrink-0">
                                <div class="h-10 w-10 rounded-full overflow-hidden border-2 border-indigo-500/30">
                            ...
                            <div class="ml-3">
                                <div class="flex items-center">
                                    <h3 class="text-base font-semibold text-white leading-tight">{{ $receiver->first_name }} {{ $receiver->last_name }}</h3>
                                    @if($receiver->isOnline())
                                        <span class="ml-2 flex h-2 w-2 rounded-full bg-green-500" title="Online"></span>
                                    @endif
                                </div>
                                <div class="flex items-center text-xs text-gray-400 mt-0.5">
                                    ...
                                    @else
                                        <span class="opacity-75">Active Now</span>
                                    @endif
                                </div>
                            </div>
                            <div class="ml-auto flex items-center space-x-1 relative">
                                <div id="mute-indicator" class="hidden p-2 text-yellow-500" title="Notifications muted">
                                    <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5.586 15H4a1 1 0 01-1-1v-4a1 1 0 011-1h1.586l4.707-4.707C10.923 3.663 12 4.109 12 5v14c0 .891-1.077 1.337-1.707.707L5.586 15z" />
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2" />
                                    </svg>
                                </div>
                                <button type="button" id="chat-menu-button" class="p-2 rounded-full text-gray-400 hover:text-white hover:bg-gray-700 transition-all duration-200" title="Conversation settings">
                                    <svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 5v.01M12 12v.01M12 19v.01M12 6a1 1 0 110-2 1 1 0 010 2zm0 7a1 1 0 110-2 1 1 0 010 2zm0 7a1 1 0 110-2 1 1 0 010 2z" />
                                    </svg>
                                </button>
                                <div id="chat-menu-dropdown" class="hidden absolute right-0 top-full mt-1 w-44 bg-gray-900 rounded-lg shadow-2xl border border-gray-700 overflow-hidden" style="z-index: 100;">
                                    <div class="py-1">
                                        <a href="{{ route('profile.show', $receiver) }}" class="flex items-center px-4 py-2 text-sm text-gray-200 hover:bg-gray-800 transition-colors no-underline !text-gray-200" style="color: #e5e7eb !important;">
                                            <span>View Profile</span>
                                        </a>
                                        <button type="button" id="search-messages-btn" class="flex items-center w-full px-4 py-2 text-sm text-gray-200 hover:bg-gray-800 transition-colors text-left">
                                            <span>Search Conversation</span>
                                        </button>
                                        
                                        <button type="button" id="call-user-btn" class="flex items-center w-full px-4 py-2 text-sm text-gray-200 hover:bg-gray-800 transition-colors text-left">
                                            <span>Call</span>
                                        </button>
                                        
                                        <button type="button" id="mute-notifications-btn" class="flex items-center w-full px-4 py-2 text-sm text-gray-200 hover:bg-gray-800 transition-colors text-left">
                                            <span>Mute Notifications</span>
                                        </button>
                                        <button type="button" id="clear-chat-btn" class="flex items-center w-full px-4 py-2 text-sm text-gray-200 hover:bg-gray-800 transition-colors text-left">
                                            <span>Clear History</span>
                                        </button>
                                        <button type="button" id="restrict-user-btn" class="flex items-center w-full px-4 py-2 text-sm text-gray-200 hover:bg-gray-800 transition-colors text-left">
                                            <span>Restrict User</span>
                                        </button>
                                        <button type="button" id="report-user-btn" class="flex items-center w-full px-4 py-2 text-sm text-gray-200 hover:bg-gray-800 transition-colors text-left">
                                            <span>Report User</span>
                                        </button>
                                        
                                        @if(auth()->user()->hasBlocked($receiver->id))
                                            <button type="button" id="unblock-user-btn" class="flex items-center w-full px-4 py-2 text-sm text-green-400 hover:bg-gray-800 transition-colors text-left">
                                                <span>Unblock User</span>
                                            </button>
                                        @else
                                            <button type="button" id="block-user-btn" class="flex items-center w-full px-4 py-2 text-sm text-red-500 hover:bg-gray-800 transition-colors text-left">
                        ...
                        <!-- Chat messages area with its own scrollbar -->
                        <div id="messages" class="flex-1 overflow-y-auto bg-gray-900 custom-scrollbar">
                            <div class="space-y-6" id="messages-inner">
                        ...
                        <button id="scroll-to-bottom-btn" class="hidden absolute bottom-24 right-8 bg-indigo-600 text-white p-3 rounded-full shadow-lg hover:bg-indigo-700 transition-all duration-300 z-10 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                            <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
<script>
    ...
    if (chatMenuButton && chatMenuDropdown) {
        ...
        // Restrict User (fully functional for admins)
    ...
    // Function to handle restrict action
    async function handleRestrictAction() {
        ...
        const typeChoice = prompt('Restriction Type:\n\n1. Content Only\n2. Messaging Only\n3. Full Restriction\n4. Temporary Suspension\n\nEnter number (1-4):');
        if (!typeChoice) return;

        const choice = parseInt(typeChoice.trim());
        if (choice < 1 || choice > 4) {
            alert('Invalid selection');
            return;
        }

        const reason = prompt('Please provide a reason for the restriction (required):');
        if (!reason || !reason.trim()) {
            alert('A reason is required.');
            return;
        }

        const days = prompt('Duration in days (optional, leave empty for permanent):');
        const durationDays = days ? parseInt(days.trim()) : null;

        try {
            const response = await fetch(`/users/${receiverId}/restrict`, {
                method: 'POST',
                headers: {
    ...
    function playRingTone() {
        ...
        // Second tone - 880Hz (slight delay)
    ...
    function startRinging() {
        ...
        if (callAvatar) {
            callAvatar.classList.add('animate-pulse', 'ring-4', 'ring-indigo-500', 'ring-opacity-50');
        }
        
        // Animate status text
        animateStatusText('Ringing');
    }
    ...
    function stopRinging() {
        ...
        const callAvatar = document.getElementById('call-avatar');
        if (callAvatar) {
            callAvatar.classList.remove('animate-pulse', 'ring-4', 'ring-indigo-500', 'ring-opacity-50');
        }
    }

    // Log call to conversation
    async function logCallToConversation(callType, duration, status) {
        const callMessage = `[${callType === 'video' ? '📹 Video' : '📞 Voice'} Call] ${status}${duration ? ' - ' + duration : ''}`;
        
        try {
            await fetch(`/messages/${receiverId}/call-log`, {
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': csrfToken,
                    'Accept': 'application/json',
    ...
    function animateStatusText(baseText) {
        ...
        statusAnimationInterval = setInterval(() => {
            ...
            dots = (dots + 1) % 4;
            if (statusEl) statusEl.textContent = baseText + '.'.repeat(dots);
        }, 500);
    }
    ...
    async function initiateCall(type) {
        ...
        try {
            ...
            if (data.status === 'success') {
                ...
                // Start ringing sound and animation
                startRinging();
    ...
    async function endCall() {
        ...
        // Log the call to conversation
        const callStatus = document.getElementById('call-status').textContent;
        const wasConnected = callStatus === 'Connected';
        const duration = wasConnected ? document.getElementById('call-timer')?.textContent || '' : '';
        const status = wasConnected ? 'Ended' : 'Missed';
        
        if (currentCallId) {
            await logCallToConversation(callType, duration, status);
        }
    ...
    if (messageForm && messageInput) {
        messageForm.addEventListener('submit', async function(e) {
            ...
            const messageHtml = `
                <div id="${tempId}" class="message-item justify-end flex relative group">
                    <div class="message-bubble sent rounded-2xl px-4 py-2 relative">
                        <p class="text-sm text-white">${escapeHtml(message)}</p>
                        <div class="flex items-center justify-end mt-1 space-x-1">
                            <span class="text-xs">${new Date().toLocaleTimeString([], {hour: 'numeric', minute: '2-digit'})}</span>
                            <span class="text-xs message-status sent">
                                <i class="fas fa-clock"></i>
                            </span>
                        </div>
                    </div>
            ...
            const noMessagesEl = messagesContainer.querySelector('.h-full.flex');
            if (noMessagesEl) {
                noMessagesEl.remove();
            }
            ...
            try {
                ...
                if (response.ok && data.status === 'success') {
                    ...
                    if (tempEl) {
                        ...
                        if (statusEl) {
                            statusEl.className = 'text-xs message-status delivered';
                ...
                if (tempEl) {
                    const statusEl = tempEl.querySelector('.message-status');
                    if (statusEl) {
                        statusEl.className = 'text-xs message-status text-red-500';
    ...
    function appendMessage(msg) {
        ...
        let html = `
            <div class="message-item ${isMine ? 'justify-end' : 'justify-start'} flex relative group">
                ${!isMine ? `<div class="flex-shrink-0 h-8 w-8 rounded-full overflow-hidden mr-3">
                    <div class="h-full w-full bg-gray-700 flex items-center justify-center text-blue-400 text-sm font-medium">
                        ${msg.sender?.first_name?.charAt(0)?.toUpperCase() || '?'}
                    </div>
                </div>` : ''}
                <div class="message-bubble ${isMine ? 'sent' : 'received'} rounded-2xl px-4 py-2 relative">
                    <p class="text-sm ${isMine ? 'text-gray-900' : 'text-white'}">${escapeHtml(msg.content || msg.message)}</p>
                    <div class="flex items-center justify-end mt-1 space-x-1">
                        <span class="text-xs">${new Date(msg.created_at).toLocaleTimeString([], {hour: 'numeric', minute: '2-digit'})}</span>
                        ${isMine ? `<span class="text-xs message-status ${msg.delivery_status || 'delivered'}">
                            <i class="fas fa-check${msg.delivery_status === 'read' ? '-double' : ''}"></i>
                        </span>` : ''}
                    </div>
                </div>
            </div>
