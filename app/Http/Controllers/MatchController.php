...
class MatchController extends Controller
{
    ...
    private function getFilteredPotentialMatches($user, $interactedUserIds, string $filter): \Illuminate\Support\Collection
    {
        ...
        return $potentialMatches->map(function($potentialMatch) use ($user) {
            ...
            $existingLike = RoommateMatch::where('user_id', $user->id)
    ...
    public function update(Request $request, string $id)
    {
        ...
        if ($request->status === 'accepted') {
            ...
            if ($mutualMatch) {
                ...
                $user1Id = min($user->id, $match->user_id);
                $user2Id = max($user->id, $match->user_id);
                ...
                $welcomeMessage = new \App\Models\Message([
                    ...
                    'content' => "Hi there! I've accepted your match request. Let's chat!",
                    'message_type' => 'text',
                ...
                $conversation->update(['last_message_at' => $welcomeMessage->created_at]);
