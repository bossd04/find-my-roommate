...
class ProfileController extends Controller
{
        ...
        try {
            ...
            $user->preference()->updateOrCreate(
                ...
                [
                    ...
                    'study_habit' => $validated['study_habit'],
                    'noise_tolerance' => $validated['noise_tolerance'],
                    'smoking' => $validated['smoking'] ?? 'never',
                    'pets' => $validated['pets'] ?? 'none',
                    'overnight_visitors' => $validated['overnight_visitors'] ?? 'with_notice',
                    'schedule' => $validated['schedule'] ?? 'irregular',
    ...
    public function requestDeletion(Request $request)
    {
        try {
            ...
            return response()->json([
                'success' => true,
                'message' => 'Account deletion request submitted successfully. An administrator will review your request.',
            ...
            \Log::error('ProfileController::requestDeletion error', [
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString()
            ]);
            ...
            return response()->json([
                'success' => false,
                'message' => 'Error submitting deletion request: ' . $e->getMessage()
            ], 500);
        }
    }
     ...
     */
    public function cancelDeletionRequest(Request $request)
    {
        try {
            $user = $request->user();
            ...
            \Log::error('ProfileController::cancelDeletionRequest error', [
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString()
            ]);
            ...
            return response()->json([
                'success' => false,
                'message' => 'Error cancelling deletion request: ' . $e->getMessage()
            ], 500);
        }
    }

    /**
     * Get the current account deletion request status for the user.
     */
    public function getDeletionRequestStatus(Request $request)
    {
        try {
            $user = $request->user();
            ...
            return response()->json([
                ...
                'can_cancel' => $deletionRequest->isPending(),
            ]);

        } catch (\Exception $e) {
            \Log::error('ProfileController::getDeletionRequestStatus error', [
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString()
            ]);
            ...
            return response()->json([
                'success' => false,
                'message' => 'Error fetching deletion request status: ' . $e->getMessage()
            ], 500);
        }
    }
}
