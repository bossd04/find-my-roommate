...
Route::middleware(['auth', 'check.user.active'])->group(function () {
    ...
    Route::prefix('messages')->group(function () {
        ...
        Route::post('/{user}/call-log', [\App\Http\Controllers\MessageController::class, 'logCall'])->name('messages.call-log');
    });
    ...
    Route::post('/account-deletion-request', [ProfileController::class, 'requestDeletion'])
        ->name('account.deletion.request')
        ->middleware(['auth']);
    Route::post('/account-deletion-request/cancel', [ProfileController::class, 'cancelDeletionRequest'])
        ->name('account.deletion.cancel')
        ->middleware(['auth']);
    Route::get('/account-deletion-request/status', [ProfileController::class, 'getDeletionRequestStatus'])
...
Route::get('/chatbot/listings', [ChatbotController::class, 'getListings'])->name('chatbot.listings')->middleware('auth');
Route::get('/chatbot/map-users', [ChatbotController::class, 'getMapUsers'])->name('chatbot.map-users')->middleware('auth');
Route::post('/chatbot/message', [ChatbotController::class, 'chat'])->name('chatbot.message')->middleware('auth');
Route::post('/chatbot/clear', [ChatbotController::class, 'clearHistory'])->name('chatbot.clear')->middleware('auth');
Route::post('/chat/send', [ChatController::class, 'send'])->middleware('auth');

// Chat API Route - Gemini AI backend
Route::post('/chat', [ChatbotController::class, 'chat'])->name('chat.api.message')->middleware('auth');
Route::post('/chat/stream', [ChatbotController::class, 'chatStream'])->name('chat.api.stream')->middleware('auth');

Route::post('/chat/test', function(Request $request) {
    return response()->json([
        'reply' => 'Test response: ' . $request->input('message', 'No message')
    ]);
})->middleware('auth');

// Debug route to check API keys
Route::get('/chatbot/debug', function() {
    $dsKey = config('services.deepseek.api_key');
    $orKey = config('services.openrouter.api_key');
    
    // Test actual API call to OpenRouter (free model)
    $testResult = null;
    if (!empty($orKey)) {
        try {
            ...
            if ($response->successful()) {
                $data = $response->json();
                $testResult = 'SUCCESS: ' . ($data['choices'][0]['message']['content'] ?? 'Empty response');
            } else {
                $testResult = 'FAILED: HTTP ' . $response->status() . ' - ' . substr($response->body(), 0, 200);
            }
        } catch (\Exception $e) {
    ...
    return response()->json([
        'deepseek_key_present' => !empty($dsKey),
        'deepseek_key_length' => $dsKey ? strlen($dsKey) : 0,
        'deepseek_key_preview' => $dsKey ? substr($dsKey, 0, 10) . '...' : null,
        'openrouter_key_present' => !empty($orKey),
        'openrouter_key_length' => $orKey ? strlen($orKey) : 0,
        'openrouter_key_preview' => $orKey ? substr($orKey, 0, 10) . '...' : null,
        'openrouter_api_test' => $testResult,
        'env_deepspeek' => env('DEEPSEEK_API_KEY') ? 'SET' : 'NOT SET',
        'env_openrouter' => env('OPENROUTER_API_KEY') ? 'SET' : 'NOT SET',
    ]);
})->middleware('auth');
